...
| Qt 绑定 | PySide6 / Qt 6 |
| 支持系统 | Windows 11，包含 150% 显示缩放 |

当前版本仅面向 Windows 11，原生标题栏与缩放优化使用 Windows DWM API；Linux 与
macOS 不属于支持目标。安装和开发本插件都不需要 IDA SDK。

已提供 [v1.1.0-beta.2 测试版](https://github.com/PoP-Lin/IDAPro-MuiLs/releases/tag/v1.1.0-beta.2)，
...
参加测试可从预发布页面下载 `IDAPro-MuiLs-v1.1.0-beta.2.zip`，安装步骤相同。
...
Windows Python 3.10/3.11 上运行检查；推送 `v1.0.0` 格式的标签后，会自动构建并
发布对应版本。
